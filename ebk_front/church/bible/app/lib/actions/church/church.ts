...
export const sendChurchMessageApi = async (data: any, id: number) =>
  HttpRequest(`eglise/eglise/contact/${id}`, "POST", data);
...
export const findChurchPaginatedApi = async (page: number) =>
  await HttpRequest(`eglise?page=${page}&limit=200`, "GET");
...
export const findCommentByForumApi = async (forumId: number) =>
  await HttpRequest(
    `comment/findByContentId/paginated/${forumId}/sujetForum?page=1&limit=100000`,
    "GET"
  );
