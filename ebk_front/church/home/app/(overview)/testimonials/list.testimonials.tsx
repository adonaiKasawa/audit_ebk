...
export default function ListeTestmonials({
  ...
  const [testimonials, setTesmonials] = useState<ItemTesmonial[]>(initData.items);
  const [selected, setSelected] = useState<string | number>("approved");
  const [, setPending] = useState<boolean>(false);
  const [totalTesmonial, setTotalTestmonial] = useState<{ pending: number; approved: number; dismiss: number; }>({
    pending: 0,
    approved: 0,
    dismiss: 0,
  });
  ...
  const countTestmonial = useCallback(() => {
    const approved = testimonials.filter(item => item.status === TestimonialStatusEnum.ACTIVE).length;
    const dismiss = testimonials.filter(item => item.status === TestimonialStatusEnum.INACTIVE).length;
    const pending = testimonials.filter(item => item.status === TestimonialStatusEnum.PENDING).length;
    setTotalTestmonial({ approved, dismiss, pending });
  }, [testimonials]);

  const handleFindTestmonials = async () => {
    setPending(true);
    const req = await findFilesByChurchPaginatedApi(TypeContentEnum.testimonials, session.user.eglise.id_eglise);
    setPending(false);
