...
export default function FinanceIncomeSsrTableUI({
  initData,
  session,
}: {
  session: Session;
  initData: ManagementIncome[];
}) {
  ...
  const renderCell = React.useCallback(
    (income: Incomes, columnKey: React.Key) => {
      switch (columnKey) {
        case "date":
          return <p>{moment(income.createdAt).format("DD-MM-YYYY")} </p>;
        case "source":
          return <p> {income.source}</p>;
        case "amount":
  ...
  const handleFindIncome = async () => {
    if (session) {
      const find = await findManagementIncomeByEgliseIdApi(
