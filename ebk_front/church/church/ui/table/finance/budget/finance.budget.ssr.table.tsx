...
export default function FinanceBudgetSsrTableUI({
  initData,
  session,
}: {
  session: Session;
  ...
  const dt = moment(
    initData.budget[initData.budget.length - 1].period.toString(),
  )
    .format("YYYY-MM-DD")
    .toString();
  const dte = moment(initData.budget[0].period.toString())
    .format("YYYY-MM-DD")
    .toString();
  ...
  const [budgetAccomplishedTotal, setBudgetAccomplishedTotal] =
    useState<number>(0);
  const [budgetAaccomplishedMonth, setBudgetAaccomplishedMonth] =
    useState<number>(0);
  const [dateBigin, setDatBigin] = useState<
  ...
  const renderCell = React.useCallback(
    (budget: Budgets, columnKey: React.Key) => {
      let income: number = 0;
      ...
      switch (columnKey) {
          ...
          return (
            <div>
              <p className="line-clamp-3">
                {moment(budget.period).format("MM-YYYY")}{" "}
              </p>
            </div>
          );
        case "amount":
        ...
        case "actions":
          return (
            <ActionBudget budget={budget} handleFindBudget={handleFindBudget} />
          );
        default:
          return <></>;
      }
    ...
    if (dateBigin || dateEnd) {
      ...
      if (initData.depenses) {
        for (let i = 0; i < initData.depenses.length; i++) {
          ...
          if (
            moment(e.createdAt).isBetween(startDate, endDate, "month", "[]")
          ) {
      ...
      <div className="flex flex-col gap-4">
        <div className="grid grid-cols-1 items-center mt-4">
          ...
          <div className="flex gap-2 items-center justify-start mt-4  md:justify-end">
            <Button size="sm" variant="flat" onClick={handelStatistique}>
              <SearchIcon style={{ fontSize: 16 }} />
              Filtrer par date
            </Button>
            <Button size="sm" variant="flat" onClick={handelIntialeFilter}>
              <IoReload size={17} />
              Réinitialiser
            </Button>
            <Button
