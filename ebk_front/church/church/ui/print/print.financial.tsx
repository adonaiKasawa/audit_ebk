...
export const PayementReportingPrint = React.forwardRef(
  (
    {
      filter,
      data_print,
    }: {
      data_print: TransactionCaisse[];
      filter: {
        keyWord: string;
    ...
    return (
      <div ref={ref} className="p-4">
        <div className="flex w-full items-center justify-between">
          ...
          <div className="text-center">
            <p className="font-bold text-4xl">RAPPORT</p>
            <p className="font-meduim text-2xl">Caise</p>
          </div>
          <div className="text-rigth">
            <p>Date d&rsquo;impression: {moment().format("DD/MM/YYYY")} </p>
            {/* <p>Filtré par mot clé: {filter.keyWord} </p> */}
            <p>
              Filtré par période: Dé{" "}
              {filter.dateDebutFilter
                ? moment(filter.dateDebutFilter).format("DD/MM/YYYY")
                : "--"}{" "}
              au{" "}
              {filter.dateFinFilter
                ? moment(filter.dateFinFilter).format("DD/MM/YYYY")
                : "--"}
            </p>
          </div>
        </div>
        ...
        <div className="mt-4">
          <Table>
            <TableHeader>
              ...
              <TableColumn className="border-r border-r-white">
                Date
              </TableColumn>
              <TableColumn className="border-r border-r-white">
                Type de transaction
            ...
            <TableBody>
              {data_print.map((item, index) => {
                return (
                  ...
                  >
                    <TableCell className="border-r border-r-white font-medium">
                      {index + 1}
                    </TableCell>
                    <TableCell className="border-r border-r-white">
                    ...
                    <TableCell className="border-r border-r-white text-right">
                      ${item.approvCaisse || 0}
                    </TableCell>
                    <TableCell className="border-r border-r-white text-right">
                      ${item.depense || 0}
...
PayementReportingPrint.displayName = "PayementReportingPrint";

export const BudgetReportingPrint = React.forwardRef(
  (
    {
      filter,
      data_print,
    ...
    return (
      <div ref={ref} className="p-4">
        <div className="flex w-full items-center justify-between">
          ...
          <div className="text-center">
            <p className="font-bold text-4xl">RAPPORT</p>
            <p className="font-meduim text-2xl">Budget</p>
          </div>
          <div className="text-rigth">
            <p>Date d&rsquo;impression: {moment().format("DD/MM/YYYY")} </p>
            {/* <p>Filtré par mot clé: {filter.keyWord} </p> */}
        ...
        <div className="grid grid-cols-2 justify-center gap-4 mt-4">
          <Card className="p-4">
            <p className="text-md font-medium">
              Total de budget prévu pour la période
            </p>
            <p className="text-md font-medium">$ {total_budget}</p>
          </Card>
          <Card className="p-4">
            <p className="text-md font-medium">
              Budget réaliser pour la période
            </p>
              ...
              {filterData.map((item, index) => {
                return (
                  ...
                  >
                    ...
                    <TableCell className="border-r border-r-white">
                      {moment(item.period).format("DD-MM-YYYY")}
                    </TableCell>
                    <TableCell className="border-r border-r-white">
                      {item.budgetLine}
                    </TableCell>
...
BudgetReportingPrint.displayName = "BudgetReportingPrint";
