...
import { formatBytes, getFileIcon } from "@/app/lib/config/func";
...
export default function ArchiveSsrTable(props: ArchiveProps) {
  ...
  const renderCell = React.useCallback(
    (archive: Archivage, columnKey: React.Key) => {
        ...
        case "name":
          return (
            <div className="flex items-center gap-4">
              {archive.hasOwnProperty("typeMime") ? (
                getFileIcon(archive.name, 30)
              ) : (
                <CiFolderOn size={30} />
              )}
          ...
          return (
            <div className="flex flex-col">
              ...
              <p className="text-bold text-tiny capitalize text-default-500">
                {moment(archive.updatedAt).format("ll")}
              </p>
            </div>
          );
        case "size":
          ...
          return (
            <div className="relative flex justify-end items-center gap-2">
              <ArchiveOptionActionComponent
                handelFindArchiveByEgliseId={props.handelFindArchiveByEgliseId}
                id={archive.id}
                name={archive.name}
                type={
                  archive.hasOwnProperty("typeMime") ? "Document" : "Folder"
                }
              />
            </div>
          );
      ...
      <div className="flex flex-col gap-4">
        <div className="flex items-center justify-between gap-3 items-end">
          ...
          <div className="flex items-center gap-3">
            <BtnSwitchArchive setState={props.setState} state={props.state} />
            <CreateFolderArchiveFormModal
              ...
              setDocumentsUrl={props.setDocumentsUrl}
              setOnUploadDocument={props.setOnUploadDocument}
        ...
        <div className="flex justify-between items-center">
          <span className="text-default-400 text-small">
            Total {props.initData.length} content
          </span>
            ...
            onDoubleClick={() => {
              router.push(
                item.hasOwnProperty("typeMime")
                  ? `${file_url}${item.path}`
                  : `/church/management/archive/${item.uuidName}`,
              );
            }}
          >
            {(columnKey) => (
