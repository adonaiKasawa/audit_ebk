...
export default function MembresSsrTableUI({
  initData,
  session,
}: {
  session: Session;
  ...
  const renderCell = React.useCallback(
    (membre: Membres, columnKey: React.Key) => {
      switch (columnKey) {
        case "nom":
          return (
            <User
        ...
        case "telephone":
          return <p>{membre.telephone} </p>;
        case "email":
          return <p>{membre.email} </p>;
        case "adresse":
          return <p>{membre.adresse}</p>;
        case "ville":
          return <p>{membre.ville}</p>;
        case "pays":
          return <p> {membre.pays}</p>;
        case "status":
          return (
            <Chip
              className="capitalize border-none gap-1 text-default-600"
              color={statusColorMap[membre.status]}
              size="sm"
              variant="dot"
            >
              {capitalize(
                statusOptions.find((item) => item.uid === membre.status)?.name,
              )}
            </Chip>
          );
        case "actions":
    ...
    const update = await updateMembreApi(
      {
        status:
          membre.status === StatusAcounteEnum.ACTIF
            ? StatusAcounteEnum.INACTIF
            : StatusAcounteEnum.ACTIF,
      },
      membre.id,
    );
