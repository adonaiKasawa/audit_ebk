...
import {
  AddAudiosFormModal,
  UpdateAudiosFormModal,
} from "@/ui/modal/form/audios";
...
export default function AudiosSsrTableUI({
  initData,
  session,
}: {
  session: Session;
  ...
  const [audios, setAudios] = useState<any>(initData.items);
  ...
  const pages = Math.ceil(audios.length / rowsPerPage);
  ...
  const filteredItems = React.useMemo(() => {
    ...
    if (
      ...
      filteredAudios = filteredAudios.filter((membre) =>
        Array.from(statusFilter).includes(membre.status),
      );
    }

    return filteredAudios;
  }, [audios, filterValue, statusFilter]);
  ...
  const renderCell = React.useCallback(
    (audio: ItemVideos, columnKey: React.Key) => {
      switch (columnKey) {
        case "lien":
          return (
            <div>
        ...
        case "titre":
          return <p>{audio.titre} </p>;
        case "vue":
          return <p>0 </p>;
        case "like":
          return <p>{audio.favoris.length}</p>;
        case "comment":
          return <p>{audio.commentaire.length}</p>;
        case "actions":
          return (
            <ActionAudios
              audio={audio}
              audios={audios}
              handleFindAudios={handleFindAudios}
              setAudios={setAudios}
            />
          );
        default:
      ...
      <div className="flex flex-col gap-4">
        <div className="flex justify-between gap-3 items-end">
          ...
          <div className="flex gap-3">
            ...
            <AddAudiosFormModal handleFindAudios={handleFindAudios} />
          </div>
        </div>
        <div className="flex justify-between items-center">
          <span className="text-default-400 text-small">
            Total {audios.length} audios
          </span>
...
export const ActionAudios = ({
  audio,
  handleFindAudios,
}: {
  audio: ItemVideos;
  setAudios: Dispatch<SetStateAction<ItemVideos[]>>;
  audios: ItemVideos[];
  ...
  const handleDeleteVideos = async () => {
    const update = await DeleteFile(audio.id, TypeContentEnum.audios);
    ...
    if (
      ...
      setAlertMsg("La suppresion de l'audio  a réussi.");
    }
  };
  ...
  return (
    <div className="relative flex justify-end items-center gap-2">
      ...
      <DialogAction
        ...
        dialogBody={<p>Étes-vous sure de vouloir supprimer cette audios?</p>}
