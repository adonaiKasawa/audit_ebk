...
export default function GestionPersonnelSsrTableUI({
  initData,
  session,
}: {
  session: Session;
    ...
    (membre: Membres, columnKey: React.Key) => {
      ...
      switch (columnKey) {
        ...
        case "actions":
          return (
            <ActionMembre
              handleFindPersonneMemebres={handleFindPersonneMemebres}
              membre={membre}
              membres={user}
  ...
  const handleFindPersonneMemebres = async () => {
    if (session) {
      const find = await findManagementPersonnelApi(
