...
export function CommentUI({
  ...
  const [initComments, setInitComments] = useState<Commentaires[] | undefined>(
    comments,
  );
...
console.log("new comment : ", commentList);
  ...
  const handleCommentFile = async () => {
    ...
    if (commentFile) {
      setCommentList((prev) => [...prev, commentFile]); // mise à jour dynamique
    ...
    if (commentFile) {
      if (inForum) {
        if (resetInitComment) {
          if (comments) {
            resetInitComment({
              ...
              links: {
                first:
                  "https://ecclesiabook.org/api_test/comment/findByContentId/paginated/1/sujetForum?limit=10000",
                previous: "",
                next: "",
        ...
        if (initComments) {
          setInitComments([...initComments, commentFile]);
        }
      }
    } else {
