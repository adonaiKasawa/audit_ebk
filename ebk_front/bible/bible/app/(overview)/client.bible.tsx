...
export function ClientBible({
  ...
  const [dataTag, setDataTag] = useState<any[]>(initData.tags);
  ...
  const setHistory = useCallback(async () => {
    ...
    try {
      await postHistoryBible({
        ...
        verse: "1", // ou le verset que tu veux
      });
...
async function setTag(tag: InputTagInterface) {
  setLoading(true);
  try {
    // POST du tag vers le backend
    await postTagBible({
      title: tag.title,
    ...
    // Mise à jour locale immédiate pour affichage sans reload
    setDataTag((prev) => [
      ...prev,
      {
        ...tag,
  ...
  useEffect(() => {
  setHistory();
}, [setHistory, chapter, book, version]);
