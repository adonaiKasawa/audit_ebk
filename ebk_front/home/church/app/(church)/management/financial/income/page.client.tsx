...
export default function ManangmentIncomePageClient({
  ...
  const [dateBigin] = useState<DateValue>(parseDate(dte));
  const [dateEnd] = React.useState<DateValue>(parseDate(dte));
  // const [, setView] = useState<"APPRO" | "DEPENS" | "all">("all");
  ...
  const handelFindAll = async () => {
    const recettes = await findManagementIncomeByEgliseIdApi(
      session.user.eglise.id_eglise,
    );
    const depenses = await findManagementExpensesByEgliseIdApi(
      session.user.eglise.id_eglise,
    );
  ...
  //           value === "APPRO"
  //             ? item.typeTransaction === "Encaissement"
  //             : item.typeTransaction === "Décaissement",
  //         )
  //       : initData;
    ...
    <div>
      ...
      <div className="flex items-center gap-4 mt-4">
        ...
        <Card className="p-4">
          <p className="text-md font-medium">
            Total decaissement de la période
          </p>
          <p className="text-md font-medium">$ {totaleExpenseMonth}</p>
        </Card>
        <Card className="p-4">
          <p className="text-md font-medium">
            Total encaissement de la période
          </p>
      ...
      <div className="flex justify-end mt-4 gap-4">
        <CreateExpenseFormModal
          handleFindExpense={handelFindAll}
          session={session}
        />
