...
export default function CardVideoFileUI({ video }: { video: ItemVideos }) {
  ...
  return (
    <Link href={`/church/videos/${video.id}`}>
      <Card
        isBlurred
        isFooterBlurred
...
export function CardVideoTutoUI({
  ...
  video: { title: string; link: string; id: number };
}) {
  // const [duration, setDuration] = useState<number>(0);
  const [imgSrc] = useState<string>(`./ecclessia.png`);
  // const [loadImgError, setLoadImgError] = useState<boolean>(false);
  ...
  return (
    <Card
      ...
      href={`/church/first-step/${video.id}`}
      isHoverable={true}
      shadow="sm"
    >
      <div
...
export function CardAudioFileUI({
  ...
  return (
    <Link href={isLink ? `/church/audios/${audio.id}` : "#"}>
      <Card
        isPressable
