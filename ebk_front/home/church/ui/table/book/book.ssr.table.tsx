...
import { AddBookFormModal, UpdateBooksFormModal } from "@/ui/modal/form/book";
...
export default function BooksSsrTableUI({
  initData,
  session,
}: {
  session: Session;
  ...
  const [book, setBooks] = useState<any>(initData.items);
  ...
  const filteredItems = React.useMemo(() => {
    ...
    if (
      ...
      filteredBooks = filteredBooks.filter((membre) =>
        Array.from(statusFilter).includes(membre.status),
      );
    }

    return filteredBooks;
  }, [book, filterValue, statusFilter]);
      ...
      <div className="flex flex-col gap-4">
        <div className="flex justify-between gap-3 items-end">
          ...
          <div className="flex gap-3">
            ...
            <AddBookFormModal handleFindBooks={handleFindBooks} />
          </div>
        </div>
        <div className="flex justify-between items-center">
          <span className="text-default-400 text-small">
            Total {book.length} book
          </span>
...
export const ActionBooks = ({
  ...
  book: ItemVideos;
  setBooks: Dispatch<SetStateAction<ItemVideos[]>>;
  books: ItemVideos[];
  ...
  const handleDeleteBooks = async () => {
    const update = await DeleteFile(book.id, TypeContentEnum.livres);
    ...
    if (
      ...
      setAlertMsg("La suppresion du livre a réussi.");
    }
  };
  ...
  return (
    <div className="relative flex justify-end items-center gap-2">
      ...
      />
      <DialogAction
        action={handleDeleteBooks}
        dialogBody={<p>Étes-vous sure de vouloir supprimer le livre?</p>}
        dialogTitle={"Suppression du livre"}
        isOpen={onBloqued}
        onClose={() => {
          setOnBloqued(false);
        }}
