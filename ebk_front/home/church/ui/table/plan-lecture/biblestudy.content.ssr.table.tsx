...
import { ItemBiblePlanLecture } from "@/app/lib/config/interface";
...
export default function BibleStudyContentSsrTableUI({
  ...
  const [bibleStudys, setBiblestudys] =
    useState<ItemBiblePlanLecture[]>(initData);
  ...
  const renderCell = React.useCallback(
    (contentsBibleStudy: ItemBiblePlanLecture, columnKey: React.Key) => {
      switch (columnKey) {
        case "#":
          return <p>1</p>;
        case "titre":
          return <p>{contentsBibleStudy.title}</p>;
        case "formation":
          return (
            <div>
              {}
              <Avatar
                alt={`${contentsBibleStudy.title}`}
                ...
                //   />
                // }
                src={`${file_url}${contentsBibleStudy.picture}`}
...
export const ActionBibleStudy = ({
  ...
  contentBibleStudy: ItemBiblePlanLecture;
