...
import { RiFileWord2Line, RiFileExcel2Line } from "react-icons/ri";
...
export function formatViewsAndDate(
  viewsCount: number,
  createdAt: string | Date,
): string {
  // Formatte le nombre de vues
  let viewsText = "";

  if (viewsCount >= 1_000_000) {
    viewsText = `${(viewsCount / 1_000_000).toFixed(1)} M vues`;
  } else if (viewsCount >= 1_000) {
    viewsText = `${(viewsCount / 1_000).toFixed(1)} k vues`;
  } else {
    viewsText = `${viewsCount} vues`;
  }

  // Format la date en fonction du type
  const dateText =
    createdAt instanceof Date
      ? moment(createdAt).fromNow()
      : moment(new Date(createdAt)).fromNow();

  return `${viewsText} · il y a ${dateText}`;
}
