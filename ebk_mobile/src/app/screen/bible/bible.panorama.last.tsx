...
const BiblePanoramaLastScreen = ({navigation}: {navigation: any}) => {
  ...
  const handleGetPanorama = useCallback(async () => {
    try {
      setLoading(true);
      const response = await findPanoramaApi(typePanoramaEnum?.Old_Testament);
...
export default BiblePanoramaLastScreen;
